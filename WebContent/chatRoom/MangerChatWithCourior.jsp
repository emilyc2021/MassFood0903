		...
		<script>
			$('.conversation').on('click', function() {
				...
				var result = jQuery.ajax({
					url : "./getCouriorHistoryChatMessage.jsp",//傳送數值之頁面
					type : "POST",
					async : false,
					data : {
						sender : strSender,
		...
		function getmessage() {
			var result = jQuery.ajax({
				url: "<%=request.getContextPath()%>/ChatRoomServlet",//傳送數值之頁面
					...
					success: function(){
						 //
						 let elementTargetDivs=document.querySelectorAll('.conversation');
						
						 for(let i = 0; i < elementTargetDivs.length ; i++){
							 let elemtntTargetDiv=elementTargetDivs[i];
							 if(elemtntTargetDiv.getAttribute('id') == $('#receiverAccountNumber').val()){
