...
<input type="hidden" name="admin"value="${adminVO.accountNumber}">
	<div>
					...
					<div id="chat-container" class="chat-container">
						...
						<div class="chat-history" id="chat-message-list">
							<c:forEach var="ChatRoomVO" items="${findMessageContent}">
										...
										<li class="clearfix other-message">
											...
											<div class="message-data align-right">
												<span class="message-data-time"> <fmt:formatDate
														value="${ChatRoomVO.dateTime}" pattern="yyyy-MM-dd HH:mm" />
												</span>
											</div>
										</li>
									</c:if>
								...
								<ul>
									<c:if test="${ChatRoomVO.messageType == 12}">
						...
						<div class="chat-message clearfix" id="chat-form">
							<input type="text" id="message-to-send"
								...
								name="action" value="merchant-response">
		...
		<script
			...
			crossorigin="anonymous"></script>
		<script src="./tfa010WebPageRelativeData/js/scripts.js"></script>
		<script src="./tfa010WebPageRelativeData/js/Customize.js"></script>
		<script
			src="https://cdn.jsdelivr.net/npm/handlebars@latest/dist/handlebars.js"></script>
		<script>
			$('.conversation').on('click', function() {
				let strSender = this.id;
				let clickSender = document.getElementById(".conversation");
				alert(clickSender);
				
				var result = jQuery.ajax({
					url : "./getMerchantHistoryChatMessage.jsp",//傳送數值之頁面
					type : "POST",
					async : false,
					data : {
						sender : strSender,
