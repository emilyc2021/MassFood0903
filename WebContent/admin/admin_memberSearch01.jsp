...
<%@ page import="com.Consumer.model.*"%>
...
%>
<%   List<ConsumerVO> list = (List<ConsumerVO>)session.getAttribute("list_m1");%> 
<%-- <jsp:useBean id="list_m1" scope="request"type="java.util.List<ConsumerVO>" /> --%>
<%-- <jsp:useBean id="deptSvc" scope="page" class="com.dept.model.DeptService" /> --%>
<%-- <jsp:useBean id="list_m1" scope="session" type="java.util.List<ConsumerVO>" /> --%>
...
<script
	...
	crossorigin="anonymous"></script>
<!-- CSS檔 -->
<link rel="stylesheet" href="./css/admin_memberSearch.css">
<!-- JS檔 -->
<script src="./js/admin_memberSearch.js"></script>
<title>後台會員查詢</title>
</head>
		...
		<section class="container">
			...
			<div class="row align-items-start">
				...
				<main class="col-lg-10 main">
					...
					<div>
						<h1>會員查詢</h1>
						<FORM METHOD="post" ACTION="member.do1">
							...
							<div class="form-inline mt-3">
								<div class="form-group col-md-6">
									<label for="memberName">會員姓名/商家名稱：</label> <input type="text"
										class="form-control" name="Name" value="">
								</div>
								<div class="form-group col-md-6">
									<label for="email">信箱：</label> <input type="text"
								...
								<div class="form-group col-md-10">
									...
									<div class="form-check form-check-inline">
										<input class="form-check-input" type="radio" name="Status"
											...
											class="form-check-input" type="radio" name="Status" id="wait"
											value="1"> <label class="form-check-label mr-2"
											for="wait">待管理員審核</label> <input class="form-check-input"
											type="radio" name="Status" id="yes" value="2"> <label
											class="form-check-label" for="yes">審核通過</label>
									</div>
								</div>
					...
					<%-- 					<jsp:include page="pages/page3.file" /> --%>
					<!-- TODO:DB查詢顯示表格 -->
						...
						<table class="table table-hover table-bordered">
							...
							<tbody>
								<c:set var="count" value="0" scope="page" />
																<c:forEach var="M1" items="${list_m1}"  begin="<%=pageIndex%>"
													end="<%=pageIndex+rowsPerPage-1%>"> 

<%-- 								<c:forEach var="M1" items="${list_m1}"> --%>
									<c:set var="count" value="${count + 1}" scope="page" />
									<tr>
										<c:set var="i" scope="session" value="0" />
										<th scope="row"><c:out value="${count}" /></th>
										<td>${M1.accountNumber }</td>
										<td>消費者</td>
										<td>${M1.name}</td>
										<td>${M1.mobilePhone}</td>
										<td>${M1.email}</td>
										<%-- <td>${M1.status}</td> --%>
										<td><c:if test="${M1.status==0}"> 不通過 </c:if> <c:if
												test="${M1.status==1}"> 待審核 </c:if> <c:if
												test="${M1.status==2}"> 通過 </c:if></td>
										<td><a
