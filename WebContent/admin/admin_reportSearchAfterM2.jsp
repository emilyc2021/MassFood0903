...
<% List orderamount=(ArrayList)request.getAttribute("orderamount"); %>
<% List orders=(ArrayList)request.getAttribute("orders"); %>
<% List orderduring=(ArrayList)request.getAttribute("orderduring"); %>
...
<!-- JS檔 -->
<script src="./js/admin_reportInfo.js"></script>
    <!-- 載入chart.js相關JS -->
...
<!-- 					查詢結果 -->
<input type=hidden id="chart-data" data-orderamount="<%=orderamount %>" data-orders="<%=orders %>" data-orderduring="<%=orderduring %>">
                        ...
                        <table class="table table-hover table-bordered">
                            ...
                            <tbody>
								...
								<c:if test="${report.merchantAccountNumber==0}">   <th scope="row"  colspan="8">本月無銷售紀錄</th></c:if>
								    <c:if test="${report.merchantAccountNumber!=0}">
                                <tr>
                                    <th scope="row"><%=++i%></th>
                                    <td>${report.merchantAccountNumber}</td>
                                    <c:forEach var="m2" items="${merchantVO.all}">
                  <c:if test="${report.merchantAccountNumber==m2.accountNumber}"><td>${m2.name}</td></c:if>
                </c:forEach>
                                    <td><%=orderMonth %></td>
                                 ...
                                 <td><button type="submit" data-toggle="modal" data-target="#ChartModal" class="btn btn-outline-primary btn-sm">每日訂單金額</button></td>
                                </tr>
                                </c:if>
                               </c:forEach>
                            </tbody>
    ...
    <div class="modal fade" id="detailModal" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <c:forEach var="list" items="${orderList}"> 
                        <dt class="col-sm-6 mb-2"><h2 class="titleh2">&#9826會員帳號：<span class="titlespan ml-1"><c:forEach var="m2" items="${merchantVO.all}">
                  <c:if test="${list.merchantAccountNumber==m2.accountNumber}">${m2.email}</c:if>
                </c:forEach></span></h2></dt>
                        <dd class="col-sm-6"></dd>
                        <dt class="col-sm-6 mb-2"><h2 class="titleh2">&#9826會員名稱：<span class="titlespan ml-1"><c:forEach var="m2" items="${merchantVO.all}">
                  <c:if test="${list.merchantAccountNumber==m2.accountNumber}">${m2.name}</c:if>
                </c:forEach></span></h2></dt>
                        <dd class="col-sm-6"><h2 style="color: #5B352C;">會員帳戶資訊<span></span></h2></dd>
                        <dt class="col-sm-6 mb-2"><h2 class="titleh2">&#9826訂單月份：<span class="titlespan ml-2"><%=orderMonth %></span></h2></dt>
                        <dd class="col-sm-6"><h2 style="color: #5B352C;">&#9830銀行代碼：<span><c:forEach var="m2" items="${merchantVO.all}">
                  <c:if test="${list.merchantAccountNumber==m2.accountNumber}">${m2.bankCode}</c:if>
                </c:forEach></span></h2></dd>
                        <dt class="col-sm-6 mb-2"><h2 class="titleh2">&#9826訂單筆數：<span class="titlespan ml-2">${list.count }筆</span></h2></dt>
                        <dd class="col-sm-6"><h2 style="color: #5B352C;">&#9830銀行帳號：<span><c:forEach var="m2" items="${merchantVO.all}">
                  <c:if test="${list.merchantAccountNumber==m2.accountNumber}">${m2.bankAccount}</c:if>
                </c:forEach></span></h2></dd>
                        <dt class="col-sm-6 "><h2 class="titleh2">&#9826應付金額：<span class="titlespan ml-2"><fmt:formatNumber value="${list.payableaccount}" type="currency" maxFractionDigits="0"/></span></h2></dt>
                        <dd class="col-sm-6"><h2 style="color: #5B352C;">&#9830銀行戶名：<span><c:forEach var="m2" items="${merchantVO.all}">
                  <c:if test="${list.merchantAccountNumber==m2.accountNumber}">${m2.bankAccountName}</c:if>
                </c:forEach></span></h2></dd>
                    </dl>
                    </c:forEach>
                   <hr style="border: 1px solid #DB7243; background-color: #DB7243;">
                    ...
                    <table class="table table-sm table-bordered table-hover">
                        <thead>
                            <tr class="table-warning ">
                                ...
                                <th scope="col" width="98px">訂單金額</th>
                                <th scope="col" width="98px">平台分潤</th>
                                <th scope="col" width="98px">應付金額</th>
                        ...
                        <% int j=0; %>
                          <c:forEach var="OK" items="${orderOK}"> 
                            <tr>
                                ...
                                <td class="edit2"><fmt:formatNumber value="${OK.mealFee }" type="currency" pattern="$###,###"  maxFractionDigits="0"/></td>
                                <td class="edit2"><fmt:formatNumber value="${OK.profitSharingAmount }" type="currency" pattern="$###,###" maxFractionDigits="0"/></td>
                                <td class="edit2"><fmt:formatNumber value="${OK.payable }" type="currency" pattern="$###,###" maxFractionDigits="0"/></td>
                                <td class="edit1"><ul><li><c:forEach var="od" items="${orderDetailVO.all}">
                  <c:if test="${OK.orderNumber==od.orderNumber}">${od.itemName}：$${od.itemPrice}&times;${od.itemAmount}<br></c:if>
                </c:forEach></li></ul></td>
                            </tr>
                            </c:forEach>
                              <tr class="table-warning ">
                    ...
                    <table class="table table-sm table-bordered table-hover">
                        ...
                        <tbody>
                         ...
                         <c:forEach var="Cancel" items="${orderCancel}"> 
                              <th scope="row"><%=++j %></th>
                                <td class="edit">${Cancel.orderNumber}</td>
                                <td class="edit"><fmt:formatDate value="${Cancel.orderDate}" pattern="yyyy-MM-dd" /></td>
                                <td class="edit2"><fmt:formatNumber value="${Cancel.mealFee }" type="currency" pattern="$###,###" maxFractionDigits="0"/></td>
                                <td class="edit2"><fmt:formatNumber value="${Cancel.profitSharingAmount }" type="currency" pattern="$###,###" maxFractionDigits="0"/></td>
                                <td class="edit2"><fmt:formatNumber value="${Cancel.payable }" type="currency" pattern="$###,###" maxFractionDigits="0"/></td>
                                <td class="edit1"><ul><li><c:forEach var="od" items="${orderDetailVO.all}">
                  <c:if test="${Cancel.orderNumber==od.orderNumber}">${od.itemName}：$${od.itemPrice}&times;${od.itemAmount}<br></c:if>
                </c:forEach></li></ul></td>
                            </tr>
                            </c:forEach>
                                      <tr class="table-danger  ">
		...
		<!-- Modal 圖表-->
    <div class="modal fade" id="ChartModal" tabindex="-1" role="dialog" aria-labelledby="modelTitleId"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title title d-flex" ><%=orderMonth %>每日訂單營業額</h5>
                    <button type="button" class="close col-1" style="margin-left: 0" data-dismiss="modal" aria-label="Close">
                        <span  aria-hidden="true">&times;</span>
                    </button>
                </div>
