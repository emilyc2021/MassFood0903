...
%>
<%   List<MerchantVO> list = (List<MerchantVO>)session.getAttribute("list_m2");%> 
<%-- <jsp:useBean id="list_m2" scope="request" type="java.util.List<MerchantVO>" /> --%>
						...
						<table class="table table-hover table-bordered">
							...
							<tbody>
								<c:set var="count" value="0" scope="page" />
																<c:forEach var="M2" items="${list_m2}"  begin="<%=pageIndex%>"
														end="<%=pageIndex+rowsPerPage-1%>"> 

<%-- 								<c:forEach var="M2" items="${list_m2}"> --%>
									<c:set var="count" value="${count + 1}" scope="page" />
									<tr>
										<c:set var="i" scope="session" value="0" />
										<th scope="row"><c:out value="${count}" /></th>
										<td>${M2.accountNumber }</td>
										<td>商家</td>
										<td>${M2.name}</td>
										<td>${M2.identityNumber}</td>
										<td>${M2.email}</td>
										<%-- <td>${M1.status}</td> --%>
										<td><c:if test="${M2.status==0}"> 不通過 </c:if> <c:if
												test="${M2.status==1}"> 待審核 </c:if> <c:if
												test="${M2.status==2}"> 通過 </c:if></td>
										<td><a
											href="adm.do?name=merchant&account=${M2.accountNumber}"><i
